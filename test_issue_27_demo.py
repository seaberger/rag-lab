...
from src.pipeline_v3.core.storage_adapters import (
    RegistryAdapter, QdrantAdapter, KeywordIndexAdapter,
    StorageArtifactsAdapter, FingerprintAdapter
)
...
async def simulate_failure_scenario():
    ...
    print(f"   Registry: {'❌ Document found (rollback failed!)' if reg_doc else '✅ Document not found (rollback success)'}")
    
    # Check vector index
    vector_state = await index_manager.verify_vector_index_state(test_doc_id)
    print(f"   Vector index: {'❌ Document found' if vector_state.get('exists') else '✅ Document not found'}")
    
    # Check keyword index
    keyword_state = await index_manager.verify_keyword_index_state(test_doc_id)
    print(f"   Keyword index: {'❌ Document found' if keyword_state.get('exists') else '✅ Document not found'}")
    
    # Check storage
    storage_path = Path("storage_data_v3") / f"{test_doc_id}.jsonl"
    print(f"   Storage artifact: {'❌ File exists' if storage_path.exists() else '✅ File not found'}")
...
async def demonstrate_successful_transaction():
    ...
    if success:
        ...
        # Check storage
        storage_path = Path("storage_data_v3") / f"{test_doc_id}.jsonl"
        print(f"   Storage artifact: {'✅ File created' if storage_path.exists() else '❌ File not found'}")
