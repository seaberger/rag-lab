...
from src.pipeline_v3.core.index_manager import IndexType
...
class TestOfficeDocuments:
    ...
    async def test_word_parsing(self):
        ...
        try:
            ...
            # Test basic parsing
            self.log_test(
                "Word document parsing", len(markdown) > 0, f"Markdown length: {len(markdown)}"
            )
            ...
            self.log_test(
                "Word content has headings", has_headings, f"Found headings: {has_headings}"
            )

            # Test table extraction
            has_tables = "|" in markdown
            self.log_test("Word table extraction", has_tables, f"Found tables: {has_tables}")

            # Test pair extraction
            self.log_test("Word pair extraction", len(pairs) > 0, f"Found {len(pairs)} pairs")
    ...
    async def test_powerpoint_parsing(self):
        ...
        try:
            markdown, pairs, metadata = await parse_powerpoint_document(
                Path(TEST_PPT_DOC), self.config
            )
    ...
    async def test_pipeline_processing(self):
        ...
        try:
            ...
            word_result = await pipeline.process_document(
                TEST_WORD_DOC, mode="auto", with_keywords=True
            )
            ...
            ppt_result = await pipeline.process_document(
                TEST_PPT_DOC, mode="auto", with_keywords=True
            )
    ...
    async def test_search_functionality(self):
        ...
        try:
            # Test keyword search
            keyword_results = pipeline.search("FieldMaxII-TOP", search_type="keyword", top_k=5)
            ...
            vector_results = pipeline.search(
                "calibration uncertainty specifications", search_type="vector", top_k=5
            )
            ...
            hybrid_results = pipeline.search(
                "ISO 17025 accreditation", search_type="hybrid", top_k=5
            )
    ...
    async def test_chunking_and_indexing(self):
        ...
        try:
            ...
            if word_doc:
                ...
                word_chunks = index_manager.get_document_chunks(word_doc.doc_id, IndexType.KEYWORD)
                ...
                if word_chunks:
                    has_keywords = any(
                        "Keywords:" in chunk.get("content", "") for chunk in word_chunks
                    )
                    self.log_test(
                        "Word chunks have keywords", has_keywords, "Keywords found in chunks"
                    )

            if ppt_doc:
                # Check PowerPoint document chunks
                ppt_chunks = index_manager.get_document_chunks(ppt_doc.doc_id, IndexType.KEYWORD)
                ...
                if ppt_chunks:
                    slide_chunks = sum(
                        1 for chunk in ppt_chunks if "Slide" in chunk.get("content", "")
                    )
                    self.log_test(
