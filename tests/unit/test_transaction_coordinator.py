...
class TestTransactionCoordinator:
    ...
    async def test_prepare_phase_failure(self):
        ...
        operation = TransactionOperation(
            operation_type=OperationType.ADD_DOCUMENT,
            doc_id="test_doc_456",
            data={"nodes": ["node1"], "content": "test", "file_path": "/test"},
        )
    ...
    async def test_commit_phase_failure(self):
        ...
        operation = TransactionOperation(
            ...
            doc_id="test_doc_789",
            data={"nodes": ["node1"], "content": "updated", "file_path": "/test"},
        )
    ...
    async def test_partial_commit_failure(self):
        ...
        operation = TransactionOperation(
            operation_type=OperationType.DELETE_DOCUMENT, doc_id="test_doc_999", data={}
        )
    ...
    async def test_concurrent_transactions(self):
        ...
        systems = [MockStorageSystem("System1", delay=0.1), MockStorageSystem("System2", delay=0.1)]
        ...
        operations = [
            TransactionOperation(
                ...
                data={"nodes": [f"node_{i}"], "content": f"content_{i}", "file_path": f"/test_{i}"},
            )
            for i in range(3)
        ]
        ...
        tasks = [coordinator.execute_transaction(op) for op in operations]
        ...
        log = coordinator.get_transaction_log()
        assert len(log) == 3
        assert {entry["doc_id"] for entry in log} == {"test_doc_0", "test_doc_1", "test_doc_2"}
