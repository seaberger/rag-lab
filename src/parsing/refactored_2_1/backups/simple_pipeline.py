...
def setup_environment(start_dir: str | None = None) -> bool:
...
def pdf_to_data_uris(pdf_path: Path, dpi: int = 150, poppler_path: str | None = None) -> list[str]:
...
def classify_document(source: str | Path, is_datasheet_mode: bool = True) -> DocumentType:
...
async def parse_datasheet_pdf(
    pdf_path: Path, prompt_text: str, model: str = "gpt-4o"
...
async def main():
    ...
    parser.add_argument(
        "--generic", action="store_true", help="Use generic mode instead of datasheet mode"
    )
