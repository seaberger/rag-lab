...
class TestIndexManager:
    ...
    def test_search_hybrid(self, index_manager):
        ...
        results = index_manager.hybrid_search(
            "test query", top_k=5, vector_weight=0.5, keyword_weight=0.5
        )
    ...
    def test_search_vector_only(self, index_manager):
        ...
        index_manager.search_vector = Mock(
            return_value=[
                {
                    "node_id": "node1",
                    "score": 0.95,
    ...
    def test_search_keyword_only(self, index_manager):
        ...
        index_manager.search_keyword = Mock(
            return_value=[
                {
                    "node_id": "node1",
                    "score": 0.95,
    ...
    def test_get_document_nodes(self, index_manager):
        ...
        index_manager.get_document_nodes = Mock(
            return_value=[
    ...
    def test_get_statistics(self, index_manager):
        ...
        # Mock registry stats
        index_manager.registry.get_statistics = Mock(
            return_value={"total_documents": 10, "by_state": {"indexed": {"count": 8}}}
        )

        # Mock vector store stats
        index_manager._get_vector_stats = Mock(
            return_value={"total_vectors": 100, "collection_size": 1024}
        )

        # Mock keyword index stats
        index_manager.keyword_index.get_stats = Mock(
            return_value={"total_documents": 10, "total_chunks": 100}
        )
    ...
    def test_search_by_metadata(self, index_manager):
        ...
        vector_results = [
            Mock(
                id="node1", score=0.9, metadata={"doc_id": "doc1", "type": "datasheet"}
            )
        ]
        ...
        index_manager._get_nodes_by_ids = Mock(
            return_value=[
                Mock(
                    id_="node1", text="Filtered result", metadata={"type": "datasheet"}
                )
            ]
        )
        ...
        results = index_manager.search_vector(
            "test query", filters={"type": "datasheet"}
        )
    ...
    def test_verify_index_consistency(self, index_manager):
        ...
        # Mock registry data
        index_manager.registry.list_documents = Mock(
            return_value=[
        ...
        index_manager._check_vector_index = Mock(
            side_effect=lambda doc_id: doc_id == "doc1"
        )
        index_manager._check_keyword_index = Mock(return_value=True)

        # Verify consistency
        index_manager.verify_consistency = Mock(
            return_value={"total_checked": 2, "inconsistent": 1, "details": []}
        )
    ...
    def test_error_handling_during_indexing(self, index_manager):
        ...
        result = index_manager.add_document(
            doc_id, "Test content", {"source": "test.pdf"}
        )
    ...
    def test_batch_operations(self, index_manager):
        ...
        for doc in documents:
            if index_manager.add_document(
                doc["doc_id"], "Test content", {"source": doc["source"]}
            ):
                results["success"] += 1
            else:
                results["failed"] += 1
