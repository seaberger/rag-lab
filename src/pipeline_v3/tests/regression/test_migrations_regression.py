    ...
    try:
        ...
        for i in range(3):
            ...
            if i == 0:
                ...
                assert result['applied_count'] == 1, f"First application should apply 1 migration"
            else:
                # Subsequent times should be no-op
                assert result['applied_count'] == 0, f"Repeat application {i+1} should apply 0 migrations"
            
            # Version should always be 1
            assert result['current_version'] == 1, f"Version should be 1, got {result['current_version']}"
...
def test_schema_migrations_table_integrity():
    ...
    try:
        ...
        migrations = [
            Migration(version=1, name="test1", up_sql="CREATE TABLE t1 (id INTEGER);", down_sql="DROP TABLE t1;"),
            Migration(version=2, name="test2", up_sql="CREATE TABLE t2 (id INTEGER);", down_sql="DROP TABLE t2;"),
            Migration(version=3, name="test3", up_sql="CREATE TABLE t3 (id INTEGER);", down_sql="DROP TABLE t3;"),
        ]
...
def test_migration_checksum_verification():
    ...
    try:
        ...
        assert verification['issues'][0]['issue'] == 'checksum_mismatch', "Should detect checksum mismatch"
