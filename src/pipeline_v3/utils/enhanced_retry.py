...
from typing import Optional, Type, Tuple, Callable, Any, List
import logging
...
class OpenAIErrorClassifier:
    ...
    def classify_error(error: Exception) -> ErrorType:
        ...
        if any(term in error_str for term in ['unauthorized', 'invalid api key', 'authentication', 'api_key']):
        ...
        if any(term in error_str for term in ['timeout', 'timed out']) or 'timeout' in error_type_name:
        ...
        if any(term in error_str for term in ['server error', 'internal error', '5', 'service unavailable']):
            return ErrorType.RETRYABLE
...
class EnhancedRetry:
            ...
            for attempt in range(1, self.config.max_attempts + 1):
                try:
                    ...
                    if self.config.timeout_escalation and 'timeout' in kwargs:
                        ...
                        escalated_timeout = original_timeout * (self.config.timeout_multiplier ** (attempt - 1))
                        kwargs['timeout'] = min(escalated_timeout, self.config.max_delay * 3)
                        logger.debug(f"Attempt {attempt}: timeout escalated to {kwargs['timeout']:.1f}s")
                    ...
                    if attempt == self.config.max_attempts:
                        logger.error(f"Function {func.__name__} failed after {attempt} attempts: {error}")
                        raise error
                    
                    # Calculate delay
                    delay = self._calculate_delay(attempt, error)
                    logger.warning(f"Attempt {attempt}/{self.config.max_attempts} failed for {func.__name__}: {error}. Retrying in {delay:.2f}s")
    ...
    def _calculate_delay(self, attempt: int, error: Exception) -> float:
        ...
        if error_type == ErrorType.RATE_LIMITED:
            ...
            logger.info(f"Rate limit detected, applying {self.config.rate_limit_backoff_multiplier}x backoff multiplier")
    ...
    def decorator(func):
        ...
        if timeout is not None:
            if asyncio.iscoroutinefunction(func):
                ...
                logger.warning("Timeout handling for sync functions not implemented in enhanced_retry_api_call")
