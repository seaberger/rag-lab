...
class OpenAIClientFactory:
    ...
    @staticmethod
    def get_api_key_info(config: Optional[PipelineConfig] = None) -> Dict[str, Any]:
        """
...
def create_vision_client(config: Optional[PipelineConfig] = None) -> OpenAI:
...
def create_text_client(config: Optional[PipelineConfig] = None) -> OpenAI:
    """Create OpenAI client optimized for text API calls (chat, embeddings)."""
    return OpenAIClientFactory.create_client(
        config=config,
        timeout=config.openai.timeout_base if config else 60
    )


def create_embedding_client(config: Optional[PipelineConfig] = None) -> OpenAI:
...
def get_openai_client(config: Optional[PipelineConfig] = None) -> OpenAI:
    """
    DEPRECATED: Use create_vision_client, create_text_client, or OpenAIClientFactory.create_client instead.
    
    Legacy function for backward compatibility.
    """
    logger.warning("get_openai_client() is deprecated. Use OpenAIClientFactory.create_client() instead.")
