...
class ChangeAnalysis:
    ...
    confidence: float
    old_fingerprint: DocumentFingerprint | None
    new_fingerprint: DocumentFingerprint | None
...
class ChangeDetector:
    ...
    def analyze_changes(
        ...
        try:
            ...
            if stored_fingerprint and change_type != ChangeType.NEW_DOCUMENT:
                ...
                content_analysis = self._analyze_content_changes(source, content, registry_doc)
    ...
    def _determine_change_type(
        ...
        stored: DocumentFingerprint | None,
        ...
        if size_change_ratio > self.structure_change_threshold:
            return ChangeType.STRUCTURE_CHANGE
        if size_change_ratio > self.major_change_threshold:
            return ChangeType.MAJOR_UPDATE
        if size_change_ratio > self.minor_change_threshold:
            return ChangeType.MINOR_UPDATE
        # Content changed but size similar - need content analysis
        return ChangeType.MINOR_UPDATE

    def _analyze_content_changes(
        self, source: str | Path, new_content: str, registry_doc: Any | None
    ) -> dict[str, Any]:
    ...
    def _compare_chunks(
        self, old_chunks: list[str], new_chunks: list[str]
    ...
    def _refine_change_type(
        self, initial_type: ChangeType, content_analysis: dict[str, Any]
    ) -> ChangeType:
        ...
        if change_ratio >= self.rewrite_threshold:
            return ChangeType.COMPLETE_REWRITE
        if change_ratio >= self.structure_change_threshold:
            return ChangeType.STRUCTURE_CHANGE
        if change_ratio >= self.major_change_threshold:
            return ChangeType.MAJOR_UPDATE
        if change_ratio >= self.minor_change_threshold or change_ratio > 0:
            return ChangeType.MINOR_UPDATE
        return ChangeType.NO_CHANGE

    def _determine_update_strategy(
        self, change_type: ChangeType, change_summary: dict[str, Any]
    ) -> UpdateStrategy:
        ...
        if change_type == ChangeType.MINOR_UPDATE and change_summary.get("changed_chunks", 0) <= 3:
    ...
    def _calculate_priority(self, change_type: ChangeType, change_summary: dict[str, Any]) -> int:
        ...
        if change_type in [
            ...
            return 1  # Urgent
        if change_type == ChangeType.MAJOR_UPDATE or change_type == ChangeType.MINOR_UPDATE:
            return 2  # Normal
        # NO_CHANGE
        return 3  # Low

    def _estimate_effort(
        self, change_type: ChangeType, content: str, affected_chunks: list[int]
    ) -> float:
    ...
    def batch_analyze_changes(self, documents: list[dict[str, Any]]) -> list[ChangeAnalysis]:
