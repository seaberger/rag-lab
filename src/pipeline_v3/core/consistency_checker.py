...
class ConsistencyChecker:
    ...
    async def check_all_documents(
        ...
        try:
            ...
            for i in range(0, len(all_doc_ids), batch_size):
                ...
                for doc_id, result in zip(batch, results):
                    if isinstance(result, Exception):
                        report.errors.append(f"Error checking {doc_id}: {str(result)}")
            ...
            report.errors.append(f"Check failed: {str(e)}")
    ...
    async def _check_document(self, doc_id: str) -> Optional[DocumentInconsistency]:
        ...
        if not presence["registry"] and any([
            presence["vector_index"], 
            presence["keyword_index"], 
            presence["storage"]
        ]):
    ...
    async def repair_inconsistencies(
        ...
        self.logger.info(
            f"Repair complete: {successful}/{len(results)} successful"
        )
    ...
    async def _repair_trust_registry(
        ...
        if doc is None:
            ...
            # Document should exist - ensure it's in all systems
            if doc.state == DocumentState.INDEXED:
                ...
                if any(system in inconsistency.missing_from for system in ["vector_index", "keyword_index"]):
