    ...
    async def commit(self, checkpoint: Checkpoint) -> bool:
        ...
        try:
            if operation.operation_type == OperationType.ADD_DOCUMENT:
                ...
                if success:
                    self.registry.update_document_state(operation.doc_id, DocumentState.INDEXED)
    ...
    async def rollback(self, checkpoint: Checkpoint) -> bool:
        ...
        try:
            if checkpoint.state_before:
                ...
                return self.registry.update_document_state(checkpoint.doc_id, state)
...
class QdrantAdapter(StorageSystem):
    ...
    async def commit(self, checkpoint: Checkpoint) -> bool:
        ...
        try:
            ...
            elif checkpoint.operation_type in [
    ...
    async def rollback(self, checkpoint: Checkpoint) -> bool:
        ...
        try:
            ...
            self.client.delete(
                collection_name=self.collection_name,
                points_selector={
                    "filter": {"must": [{"key": "doc_id", "match": {"value": checkpoint.doc_id}}]}
                },
            )
            ...
            if checkpoint.state_before and checkpoint.state_before.get("points"):
                ...
                if restore_points:
                    self.client.upsert(collection_name=self.collection_name, points=restore_points)
...
class KeywordIndexAdapter(StorageSystem):
    ...
    async def commit(self, checkpoint: Checkpoint) -> bool:
        ...
        try:
            ...
            if checkpoint.operation_type in [
                OperationType.ADD_DOCUMENT,
                OperationType.UPDATE_DOCUMENT,
            ]:
    ...
    async def verify_state(self, doc_id: str) -> dict[str, Any]:
        ...
        try:
            with self.index.get_connection() as conn:
                cursor = conn.execute("SELECT COUNT(*) FROM documents WHERE doc_id = ?", (doc_id,))
...
class StorageArtifactsAdapter(StorageSystem):
    ...
    def __init__(self, storage_dir: Path):
        ...
        self.backup_files: dict[UUID, bytes | None] = {}
    ...
    async def prepare(self, operation: TransactionOperation, operation_id: UUID) -> Checkpoint:
        ...
        if artifact_path.exists():
            checkpoint.state_before = {"exists": True, "path": str(artifact_path)}
