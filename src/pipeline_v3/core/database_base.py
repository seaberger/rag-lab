...
from .migrations import MigrationManager, Migration, load_migrations_from_sql_files
...
class DatabaseBase:
    ...
    def _run_migrations(self):
        ...
        try:
            ...
            logger.info(
                f"{self.db_name} current schema version: {current_version}"
            )
    ...
    def verify_schema(self) -> bool:
        ...
        migrations_dir = (
            Path(__file__).parent.parent / "migrations" / self.migrations_subdir
        )
        ...
        try:
            ...
            if not result['valid']:
                logger.error(
                    f"Schema verification failed for {self.db_name}: "
                    f"{result['issues']}"
                )
