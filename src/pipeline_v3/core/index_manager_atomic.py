...
from .registry import DocumentState, IndexType
from .fingerprint import FingerprintManager
from .transaction_coordinator import (
    OperationType, TransactionCoordinator, TransactionError, TransactionOperation
)
from .storage_adapters import (
    FingerprintAdapter, KeywordIndexAdapter, QdrantAdapter, 
    RegistryAdapter, StorageArtifactsAdapter
)
...
class AtomicIndexManager(IndexManager):
    ...
    def _init_transaction_support(self) -> None:
        ...
        self.transaction_coordinator = TransactionCoordinator(
            systems=list(self.adapters.values()),
    ...
    async def add_document_atomic(
        ...
        try:
                ...
                "nodes": [
                    {
                        ...
                        "embedding": node.embedding.tolist() if hasattr(node, 'embedding') and node.embedding is not None else None
                    }
                    for node in nodes
                ],
            ...
            success, errors, details = await self.transaction_coordinator.execute_transaction(operation)
