    ...
    try:
        ...
        with DocumentRegistry(config) as registry:
            ...
            try:
                ...
                print(
                    f"✅ Consistency check: {len(inconsistent)} inconsistent, {len(orphaned)} orphaned"
                )
    ...
    try:
        ...
        with IndexManager(config) as index_mgr:
            ...
            try:
                ...
                print(
                    f"✅ Consistency check completed: {consistency.get('overall_health', {}).get('status', 'unknown')}"
                )
            except Exception as e:
    ...
    try:
        ...
        with ChangeDetector(config) as detector:
            ...
            try:
                ...
                print(
                    f"✅ Change analysis completed: {analysis2.change_type.value} -> {analysis2.update_strategy.value}"
                )
                print(
                    f"✅ Processing priority: {analysis2.processing_priority}, effort: {analysis2.estimated_effort}s"
                )
                ...
                print(
                    f"✅ Update recommendations: {recommendations.get('total_documents', 0)} documents"
                )
        ...
        async with EnhancedPipeline(config) as pipeline:
            ...
            try:
                ...
                result = await pipeline.process_document(
                    source=test_docs[0], metadata={"test_id": 1}
                )
                print(f"✅ Single document processing: {result['status']}")

                # Test batch processing (direct)
                batch_data = [
                    {"source": str(test_docs[1]), "metadata": {"test_id": 2}},
                    {"source": str(test_docs[2]), "metadata": {"test_id": 3}},
                ]

                batch_result = await pipeline.process_document_batch(
                    documents=batch_data, use_queue=False, max_concurrent=2
                )
                print(
                    f"✅ Batch processing (direct): {batch_result['status']}, {batch_result.get('successful', 0)} successful"
                )

                # Test search (simplified)
                try:
                    search_results = pipeline.search(
                        query="test document", search_type="keyword", top_k=5
                    )
                ...
                print(
                    f"✅ Comprehensive status: {status.get('pipeline', {}).get('processing_stats', {}).get('documents_processed', 0)} processed"
                )

                # Test update recommendations
                recommendations = pipeline.get_update_recommendations(time_budget=30.0)
                print(
                    f"✅ Update recommendations: {recommendations.get('total_documents', 0)} recommended"
                )
