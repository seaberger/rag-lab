...
"""
        ...
        try:
            ...
            assert result.returncode == 126 or result.returncode == 1  # Allow both, depending on error handling
    ...
    def test_bad_config_path(self):
        ...
        exit_code, stdout, stderr = self.run_cli_subprocess(["--config", "nonexistent.yaml", "status", "--help"])
...
"""
    ...
    def test_value_error_handling(self):
        ...
        exit_code, stdout, stderr = self.run_cli_subprocess(["search", "test", "--top-k", "invalid"])
        assert exit_code != 0
...
"""
        ...
        try:
            ...
            log_file_line = [line for line in result.stdout.split('\n') if line.startswith('LOG_FILE:')]
            if log_file_line:
    ...
    def test_cli_argument_error_with_command_string(self):
        ...
        with patch('cli.management.main', side_effect=CLIArgumentError("Test error", command_string="status")):
            with patch('sys.exit') as mock_exit:
                from cli_main import run_cli
...
class TestCLIRegressionSubprocess:
    ...
    def test_error_scenarios_exit_codes(self):
        ...
        for cmd_args, check_func in error_scenarios:
            ...
            assert check_func(result.returncode), f"Command {cmd_args} returned unexpected exit code {result.returncode}"
